...
//        self.timer = DispatchSource.makeTimerSource(flags: DispatchSource.TimerFlags(rawValue: 0), queue: DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.default)) /*Migrator FIXME: Use DispatchSourceTimer to avoid the cast*/ as! DispatchSource
        self.timer = DispatchSource.makeTimerSource(flags: DispatchSource.TimerFlags(rawValue: 0), queue: DispatchQueue.global(qos: DispatchQoS.QoSClass.default)) /*Migrator FIXME: Use DispatchSourceTimer to avoid the cast*/ as! DispatchSource
...
//        self.timer.setTimer(start: DispatchTime.now(), interval: NSEC_PER_SEC / 300, leeway: 0)
        self.timer.scheduleRepeating(deadline: .now(), interval: .seconds(1), leeway: .seconds(1));
